...
public class CustomerRepositoryImpl extends AbstractRepository<Customer> implements CustomerRepository {
  ...
  @Override
  public Customer findSecondaryMatchedCustomer(String tsbMatchSecondary) {
    Query query = sessionFactory
			.getCurrentSession()
			.createQuery("select c from Customer c where c.bookmarksAccount.tsbMatchSecondary = :tsbMatchSecondary");
	      	query.setParameter("tsbMatchSecondary", tsbMatchSecondary);
	         return (Customer) query.uniqueResult();
  }
	...
	public Collection<Customer> getForAutoComplete(String searchString, Boolean accountHolders) {
		String baseSql = "select new Customer(c.id, c.firstName, c.lastName, c.address.postcode, c.bookmarksAccount.currentBalance) from Customer c ";
