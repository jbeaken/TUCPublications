...
@Service
public class CustomerOrderLineServiceImpl extends AbstractService<CustomerOrderLine> implements CustomerOrderLineService {
	@Autowired
	...
	@Override
	@Transactional
	public void complete(CustomerOrderLineStatus status, Long customerOrderLineId) {
	...
	public Collection<CustomerOrderLine> search(SearchBean searchBean) {
		 Collection<CustomerOrderLine> customerOrderLine = customerOrderLineRepository.search(searchBean);
	...
	public void updateToOrdered(Map<Long, Long> customerOrderLineMap) {
			for(Long id : customerOrderLineMap.keySet()) {
				customerOrderLineRepository.updateToOrdered(id, customerOrderLineMap.get(id));
			}
	}
	...
	public Collection<CustomerOrderLine> findOpenOrdersForStockItem(StockItem stockItem) {
		return customerOrderLineRepository.findOpenOrdersForStockItem(stockItem);
	}

	@Override
	public void fill(CustomerOrderLine customerOrderLine, SupplierDeliveryLine supplierDeliveryLine) {
//		Collection<SupplierDeliveryLine> supplierDeliveryLines = customerOrderLine.getSupplierDeliveryLines();
//
...
//		long amountFilled = (customerOrderLine.getAmountFilled() == null) ? 0 : customerOrderLine.getAmountFilled();
//		Long newAmountFilled = amountFilled + amountToFill;
		if(amountToFill > customerOrderLine.getAmount()) {
			//Cannot be
			throw new BookmarksException("Fill amount is higher than order amount");
		}
		...
		stockItem.setQuantityReadyForCustomer(stockItem.getQuantityReadyForCustomer() + amountToFill);

		if(customerOrderLine.getDeliveryType() == DeliveryType.MAIL) {
			customerOrderLine.setStatus(CustomerOrderLineStatus.READY_TO_POST);
		} else { 
			//Collection
			customerOrderLine.setStatus(CustomerOrderLineStatus.INFORM_CUSTOMER_TO_COLLECT);
		}
	}
	...
	/**
	 * Fill from searchCustomerOrders screen, instant fill unlike above fill methods which
	 * just prepare for createCustomerOrder
	 */
	@Override
	public void fill(CustomerOrderLine customerOrderLine) {
		long amountFilled = customerOrderLine.getAmountFilled(); //From post
		customerOrderLine = get(customerOrderLine.getId()); //Horrible amount of sql, must be easier way
		...
		if(customerOrderLine.getStatus() == CustomerOrderLineStatus.IN_STOCK) {
			...
			stockItemService.updateQuantities(customerOrderLine.getStockItem(), amountFilled * -1, null, null, null, amountFilled);
		}
	...
	@Override
	public void makeNote(Long id, CustomerOrderLineStatus customerOrderLineStatus) {
		...
		note = note + customerOrderLineStatus.getDisplayName() + " on " + sdf.format(new Date());
		col.setNote(note);
		update(col);
	}
