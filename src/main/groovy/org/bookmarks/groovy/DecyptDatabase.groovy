...
if( !args || args.length < 1) {
  println "Usage : DecryptDatabase dbPassword encPassword"
  System.exit(0)
}
...
def customers = sql.eachRow("select * from customerorderline ") { row ->
  ...
  String encc1 = encryptor.decrypt(cc1)

  String cc2 = row.creditCard2
  String encc2 = encryptor.decrypt(cc2)

  String cc3 = row.creditCard3
  String encc3 = encryptor.decrypt(cc3)

  String cc4 = row.creditCard4
  String encc4 = encryptor.decrypt(cc4)

  String sc = row.securityCode
  String encSC = encryptor.decrypt(sc)

  String month = row.expiryMonth
  String encMonth = encryptor.decrypt(month)

  String year = row.expiryYear
  String encYear = encryptor.decrypt(year)

  String nameOnCard = row.nameOnCard
  String encNameOnCard = encryptor.decrypt(nameOnCard)

  count++
  println "${count} : Decrypting id ${row.id}"
