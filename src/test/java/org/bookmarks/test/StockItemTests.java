...
public class StockItemTests extends AbstractBookmarkWebsiteTests {
    
    @Test
	public void displayAdd() throws Exception {
        ResultActions actions = this.mockMvc.perform(get("/stock/add"));
...
//        actions.andExpect(content().string(containsString("<title>Create " + getEntityViewName() + "</title>")));
	...
	public void displaySearch() throws Exception {
        ResultActions actions = this.mockMvc.perform(get("/stockItem/displaySearch"));
        actions.andDo(print()); // action is logged into the console
//        actions.andExpect(content().string(containsString("<title>Create " + getEntityViewName() + "</title>")));
